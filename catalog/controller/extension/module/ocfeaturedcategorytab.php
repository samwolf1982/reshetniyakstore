...
class ControllerExtensionModuleOcfeaturedcategorytab extends Controller {
            ...
            foreach ($_featured_categories as $_category) {
                if ($use_thumbnail) {
                    ...
                    if ($_category['thumbnail_image']) {
                        $thumbnail_image = $this->model_tool_image->resize($_category['thumbnail_image'], 199, 113);
                    } else {
                        $thumbnail_image = $this->model_tool_image->resize('placeholder.png', 199, 113);
                    }
                } else {
    ...
    public function getProductFromData($data= array(), $setting = array()) {
        ...
        foreach ($results as $result) {
            if ($result['image']) {
                $image = $this->model_tool_image->resize($result['image'], 800, 926);
            } else {
                $image = false;
            }
