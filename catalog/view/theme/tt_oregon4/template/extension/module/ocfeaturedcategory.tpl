...
<?php 
	  ...
	  if(!$rows) { $rows=1; }
   ?>
<div class="featured-categories-slider module-title"><h2><?php echo $title; ?></h2></div>
		<div class="row">
		<div class="owl-featured-categories">
			<?php foreach ($categories as $category) { ?>
			<?php  if($count % $rows == 0 ) { echo '<div class="fcategory-content">'; } $count++; ?>	
							...
							<?php if($category['children']): ?>
								...
								<ul class="sub-featured-categories">
									...
									<?php if($sub_count >= $number_sub) break; ?>
										...
										<?php $sub_count++; ?>
									<?php endforeach; ?>
									<li><a href="<?php echo $category['href']; ?>"><?php echo $text_readmore; ?></a></li>
								</ul>
							<?php endif; ?>
						<?php endif; ?>	
					</div>
				</div>
				<?php if($count % $rows == 0 || $count == count($categories)): ?>
			</div>
			<?php endif; ?>
			<?php } ?>
		</div>
